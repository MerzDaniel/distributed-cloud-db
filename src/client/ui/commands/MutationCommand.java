...
public class MutationCommand implements Command {
    ...
    public void execute(ApplicationState state) {
        ...
        try {
            ...
            writeLine(String.format("Mutate >> : '%s'", mutationMessage.prettyPrint()));
            graphMessageResponse = state.kvStore.mutate(mutationMessage);
        } catch (IOException e) {
            logger.error("error", e);
            writeLine(String.format("An Error occurred during the MUATE : %s (%d ms)", e.getMessage(), t.time()));
            return;
        ...
        if (graphMessageResponse.errorMsg != null && !graphMessageResponse.errorMsg.equals(""))
            ...
            writeLine(String.format("Data saved successfully >>: '%s' (%d ms)", graphMessageResponse.data.prettyPrint(), t.time()));
    }
